...
public class CandidateDaoReg implements ICandidate {
	...
	public CandidateDaoReg() throws ClassNotFoundException, SQLException {
       ...
       pst4=cn.prepareStatement("select name from candidates ");
    //   pst5=cn.prepareStatement("UPDATE candidates set votes = ? where id = ?");
	}
	...
	while(rst.next()) {
		candidateList.add(new Candidate(rst.getInt(1),rst.getString(2),rst.getString(2),rst.getInt(4)));
	}
	...
	@Override
	public List<String> fetchAllCandidates() throws SQLException {

		List<String> candidateList = new ArrayList<>();
	
		try (ResultSet rst = pst4.executeQuery()) {
			while (rst.next())
				candidateList.add(rst.getString(2));
		}
		return candidateList;
